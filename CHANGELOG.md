...
### Added
- MYSEN: moduł `src/showcase/mysen-merge-lyrics-from-timestamps.js` (merge intro + timestampy jak `/mysen` bez showcase JSON); `buildShowcaseAnimationDocFromMysenRuntimeMerge` w `showcase-animation-adapters.js` — dokument do autoringu; edytor showcase: preset **„MYSEN jak /mysen (intro + TS z repo)”** (`Źródło`); `lyricsArrayFromShowcaseDoc` przenosi `offsetX` / `offsetY` / `offsetZ` / `mysenSplitRow`; test `tools/mysen-runtime-showcase-doc.test.mjs` (`node --test tools/mysen-runtime-showcase-doc.test.mjs`).
- Menu terminala: sekcje **ANIMATIONS** / **GAMES** / **SOFTWARE** z nagłówkami `.term-menu-category-heading`, odstęp `.term-menu-category-spacer` przed grami; banery animacji w kolejności **Pushka** → **Newskin** → **Mysen** → **Vajbuj** (`CONFIG.pushkaMenuBannerVideo`, `PUSHKA_CONFIG`, `/pushka`); test kontraktu HTML `tools/terminal-menu-layout.test.mjs` (`node --test`).
- **Pushka Studio:** modal **`#pushka-modal`** (jak newskin: scrim, minimalizacja, `body.pushka-modal-open`), iframe Vimeo z `PUSHKA_CONFIG.vimeoUrl` (`normalizeVimeoEmbedUrl` współdzielone z portfolio); baner — klip `ASSETS/UI/pushka_baner.mp4`, neon **PUSHKA STUDIO**, nakładka zamiast START ANIMATION: **OPEN VIDEO** (`menuBannerArmedLabel` / `data-term-banner-armed-label` ustawiane z config w `initTerminalMenuHoverDrawerCopy`).
- Menu terminala: bloki **Games / Software** w `.term-menu-section-hover-block` — ta sama szuflada co banery (`grid-template-rows` hover/focus), wspólny nagłówek **`.term-menu-drawer-display-title`** (Orbitron) + opis z `TERMINAL_CONFIG.menuSectionHoverDrawers` (`script.js`: `initTerminalMenuHoverDrawerCopy`); `VAJBUJ_CONFIG.menuBannerDrawerDisplayTitle` dla neonowego tytułu w szufladzie stripu VAJBUJ.
- Banery VAJBUJ / MYSEN / NEWSKIN / Pushka w menu terminala: **dwuetapowe** uruchomienie — domyślna angielska etykieta **START ANIMATION** (`TERMINAL_CONFIG.menuBannerStartAnimationLabelEn`); opcjonalnie `data-term-banner-armed-label` + `data-term-banner-arm-second-hint` (Pushka: **OPEN VIDEO**); po pierwszym kliknięciu klasa `--armed`; drugi klik → `submitLine` z `data-term-banner-cmd`; `.term-menu-banner-armed-overlay` w `index.html`.
- Menu terminala: trzeci baner wideo **NEWSKIN** (`CONFIG.newskinMenuBannerVideo`, domyślnie `ASSETS/UI/newskin_baner.mp4`) — ten sam mechanizm co VAJBUJ (pierwsza klatka w spoczynku, hover/focus = odtwarzanie + nakładka z tekstem `/newskin`); `object-position` pod przycięcie szerokiego klipu; `newskin-active` na `#term-newskin-banner-hint` gdy modal `/newskin` jest otwarty.
- Po naturalnym zakończeniu VAJBUJ / MYSEN: modal z embedem YouTube oryginału (`VAJBUJ_CONFIG` / `MYSEN_CONFIG`: `originalTrackYoutubeEmbedSrc`, tytuł, krótki tekst po angielsku); pierwszeństwo przed opcjonalnym widgetem Spotify; zamykanie Esc / X / tło; ukrywanie przy starcie trybu i przy `forceStop*Silent`.
- Konsola: komenda `/newskin` — podmiana źródła **tego samego** wideo tła 3D (`backgroundVideoEl` / `backgroundVideoMesh`) na klip z `NEWSKIN_CONFIG` (skala płaszczyzny, `mapColorGain`, opcjonalne boosty IBL), więc obraz przechodzi przez **EffectComposer** i **fake GI** jak zwykłe BG; panel YouTube jak sekcja menu (280px, tokeny `--menu-surface-*`), wyśrodkowany, z **minimalizacją** (chevron); napis sceny → `NEWSKIN_CONFIG.displayText` generowany **helvetiker regular** (cienki font); konsola obsługuje wynik komendy jako `Promise`; `setBackgroundVideoBySrc` jest `async` (przy otwartym newskin najpierw zamyka modal).
- Menu terminala: baner MYSEN (`CONFIG.mysenMenuBannerVideo`, `CONFIG.mysenMenuBannerPoster`) — w stanie spoczynku statyczny plakat (`ASSETS/UI/mysen_menu_banner_poster.png`), pod filmem przycięta pętla z ostatnich 3 s źródła `ASSETS/UI/mysen_baner.mp4` (`mysen_menu_banner_last3s.mp4`); po najechaniu/fokusie znikanie plakatu, odtwarzanie wideo, szuflada z neonem **MYSEN** i podpowiedzią; `mysen-active` na `#term-mysen-banner-hint` i wrapperze `#term-mysen`.
- Menu terminala: baner wideo VAJBUJ (`CONFIG.vajbujMenuBannerVideo`, domyślnie `ASSETS/UI/Vajbuj_baner.mp4`) — pierwsza klatka w stanie spoczynku, nagłówek i podpowiedź konsoli jako nakładka na filmie, po najechaniu (lub fokusu klawiatury) odtwarzanie z pętlą oraz lekki efekt „twitch” (wyłączany przy `prefers-reduced-motion`).
- Edytor showcase (`tools/showcase-animation-editor`): oś czasu z zoomem względem szerokości okna (1× = brak poziomego scrolla), zoom kółkiem i pan środkowym przyciskiem myszy; `TransformControls` (przesuw / obrót, world/local) dla zaznaczonego słowa z zapisem do `transformKeyframes`; zwarty panel Px–Pz / Rx–Rz z przyciskami klucza per parametr i `upsert` przy `t`; wideo tła pod podglądem (plik + `style.backgroundVideo`: `relativePath`, `opacityKeyframes`, `playbackRateKeyframes`) zsynchronizowane z audio; eksport JSON przez `cloneDocForJsonExport` bez URL-i tymczasowych.
- Dokument pusty / migracja v2 (`showcase-animation-schema.js`): domyślne `style.backgroundVideo` z pustą ścieżką i pustymi tablicami kluczy.

### Changed
- Modal **Pushka Studio** (`#pushka-modal`): szerszy panel (`min(960px, 100vw − 48px)`), żeby standardowy embed Vimeo i jego kontrolki mieściły się bez ścisku; na bardzo wąskim ekranie pełna szerokość minus margines.
- Menu terminala: usunięto duplikat bloku MYSEN pod banerami; podpowiedź `/mysen` jest w szufladzie banera MYSEN; neon **MYSEN** jak u pozostałych banerów (`term-menu-drawer-display-title`, `MYSEN_CONFIG.menuBannerDrawerDisplayTitle`); z paneli Games/Software usunięto wewnętrzne `.menu-section-title` (tytuły tylko jako nagłówki kategorii); uproszczono opisy w szufladach (bez powielania nazwy z neonem); `#term-mysen` przeniesiono na wrapper banera MYSEN (`mysen-active` = obramowanie stripa).
- Lewy panel (`#camera-hud`): wszystkie sekcje `.controls-section` są domyślnie rozwinięte (wcześniej zwinięte były wszystkie poza kamerą i „Mouse animation Mode”).
- Baner **NEWSKIN** w menu: usunięto nakładkę na wideo (`.newskin-menu-banner-brand`); ten sam styl nagłówka co wcześniej jest w szufladzie jako **`.term-menu-drawer-display-title`** (współdzielony z VAJBUJ i sekcjami dolnymi); stan **newskin-active** podświetla nagłówek i hint w szufladzie.
- Banery w menu terminala (`index.html`, `style.css`, `script.js`): tekst opisu nie nakłada się na wideo — **szuflada** (`.term-menu-banner-drawer`) wysuwa się **pod** paskiem wideo (`grid-template-rows: auto 0fr` → `1fr` przy `:hover` / `:focus-within` na `.term-menu-banner-block`), powiększa kartę banera i **przesuwa** niższe sekcje menu; styl szuflady: `.term-menu-banner-drawer-inner`; strip VAJBUJ: neonowy tytuł w szufladzie zamiast `.term-menu-banner-drawer-title`; NEWSKIN: klasa `.newskin-menu-banner-drawer-hint`; odtwarzanie wideo / „twitch” / jasność reagują na **cały blok** (mysz i `focusin`/`focusout`), żeby najechanie na tekst nie zatrzymywało klipu.
- Banery w menu terminala (`index.html`, `style.css`, `script.js`): wspólny prefiks klas **`.term-menu-banner-*`** zamiast mylącego `.mysen-menu-banner-*` (VAJBUJ nie jest MYSEN); klip VAJBUJ: **`#vajbuj-menu-banner-video`**, wrapper **`.term-menu-banner-wrap--vajbuj`**; `initVajbujMenuBannerVideo` szuka tego id i `closest('.term-menu-banner-wrap--vajbuj')`; animacja „twitch”: `@keyframes term-menu-banner-twitch`.
- Banery terminala (`style.css`): nakładka z tytułem i podpowiedzią — `visibility` zsynchronizowana z `opacity`, bez wymuszania widoczności przy `prefers-reduced-motion` (tekst jak wcześniej tylko po hoverze / `:focus-visible` na stripie; opis w `aria-label` na opakowaniu).
- Baner **NEWSKIN** w menu (`index.html`, `style.css`): znacznie jaśniejsze wideo w spoczynku i na hoverze (`brightness` + `saturate` + `contrast`); duży napis **NEWSKIN** (Orbitron) w szufladzie po hoverze/fokusie wraz z podpowiedzią terminala; wzmocnienie blasku nagłówka gdy otwarty modal `/newskin`; kadrowanie pierwszej klatki `object-position: 50% 50%`.
- `WYTYCZNE_PROJEKTU.md`: wyjątek CDN — Google Fonts (Orbitron) dla **nagłówków szuflad** w menu terminala (nie tylko pojedynczy baner).
- Banery wideo w menu terminala (`style.css`, `index.html`): wspólny `aspect-ratio: 926 / 243` (¾ wysokości względem ramy 926×324) + `object-fit: cover` na wideo; nakładka z tekstem (`.term-menu-banner-overlay--until-hover`) domyślnie ukryta także na **VAJBUJ** — widoczna po hoverze/fokusie jak na MYSEN; przy `prefers-reduced-motion` nakładka pozostaje ukryta do hovera/fokusu (opis w `aria-label`).
- Baner VAJBUJ w menu (`index.html`, `style.css`): nagłówek **Vajbuj** i linia „Type in terminal: vajbuj…” w `.term-menu-banner-overlay` na filmie (gradient u dołu); `#term-vajbuj` bez klasy `term-line`, żeby animacja intro (`post-intro-menu-line-glitch`) nie nakładała i nie psuła tekstu; `vajbuj-active` → podświetlenie przez `:has(#term-vajbuj.vajbuj-active)` na overlay.
- Baner wideo w menu terminala (`style.css`): domyślnie `brightness(1/3)`, pełna jasność przy hoverze/focusie; `isolation` + `z-index` oraz „twitch” na `<video>` — kolejne sekcje menu nie nakładają się na film.
- Baner MYSEN w menu (`style.css`): w stanie spoczynku widać **plakat** nad klipsem — dodano `brightness(1/3)` na `.term-menu-banner-poster`, żeby przyciemnienie było takie jak przy VAJBUJ (wcześniej tylko ukryte wideo było przyciemnione).
- Edytor showcase (`showcase-animation-editor.js`, `mysen-timestamp-parse.js`): import **Whisper/TS** — słowa z `groupedLine` (linie na szynie, bez losowego latania); po imporcie jeden klucz `transformKeyframes` na słowo przy `startTime` (offset tożsamościowy), żeby linie były wyśrodkowane jak w `layoutRailPositions`; heurystyka szerokości słowa gdy font jeszcze nie załadowany; klik słowa na liście ustawia czas na najwcześniejszy klucz transformacji tego słowa.
- Podgląd edytora showcase (`editor-preview-scene.js`): renderer z przezroczystością pod warstwą wideo.
- Intro **AGENTS** (`ASSETS/agents/`): pierwszy akapit — wyróżnienie **„2 LAT”** i **„15-20%”** (pogrubienie, czerwień); kolumna intro przesuwa się w pionie po każdym bloku tak, by środek nagłówka **„A G E N T S”** trafiał w środek widoku; `resize` z debounce ponownie wyśrodkowuje.
- Intro **AGENTS** (`ASSETS/agents/`): po zbudowaniu trzech linii tekstu duży nagłówek **„A G E N T S”** wchodzi powoli (opacity + scale); dopiero potem pojawia się CTA do symulacji.
- Intro **AGENTS** (`ASSETS/agents/`): fazy każdej linii — pisanie co najmniej ~8 s, pauza ~8 s przed kolejną linią; wszystkie linie zostają widoczne (bez znikania/czyszczenia); CTA po krótkiej pauzie na końcu.
- Intro **AGENTS**: dłuższe tempo CTA (transition); długość wejścia tytułu **„A G E N T S”** = `--agents-hero-reveal-duration` w CSS, odczytywane w JS (bez sztywnego przedwczesnego timeoutu).

### Fixed
- Edytor showcase (`showcase-animation-editor.js`): dopisano import `parseMysenTimestampLyricsFile` dla presetu „MYSEN jak /mysen” (wcześniej `ReferenceError` przy ładowaniu timestampów).
- `/newskin` (`script.js`): ×, scrim i Esc zamykają tylko panel YouTube — klip tła 3D i napis NEWSKIN zostają; pełny powrót dopiero po zmianie tła w HUD (`setBackgroundVideoBySrc` czyści tryb newskin przy `newskinBackgroundRestore`, także gdy modal jest ukryty); usunięto zbędną regenerację voxelowego napisu przy zamknięciu okna (źródło zawieszeń).
- Edytor showcase (`showcase-animation-editor.js`): przywrócono brakujący import `parseWhisperStyleTimestampTxtToWordEntries` z `mysen-timestamp-parse.js` (naprawa `ReferenceError` przy wgrywaniu Whisper/TS).
- Edytor showcase (`showcase-animation-editor.js`): usunięto wywołanie `transformControls.update()` w pętli `animate` — w Three.js 0.160 `TransformControls` nie ma metody `update()` (w przeciwieństwie do `OrbitControls`), co powodowało `TypeError`.
- Edytor showcase (`tools/showcase-animation-editor.js`): ścieżki `ASSETS/…` i `?doc=ASSETS/…` oraz fetch z `MYSEN_CONFIG` rozwiązywane względem rootu repozytorium (`import.meta.url`), żeby nie trafiać w `/tools/ASSETS/…` (404); strona edytora: `link rel="icon"` jak na `index.html`, żeby ograniczyć żądanie `/favicon.ico`.
- Intro **AGENTS**: wyśrodkowanie kolumny — korekta `translateY` bez chowania transformu na `0` przed pomiarem (usuwało skok w dół przy kolejnych blokach i przy animacji „A G E N T S”); przy `resize` pełne przeliczenie od zera.
- Intro **AGENTS**: sztywny timeout w JS kończył sekwencję przed dłuższą transition nagłówka — zmiana `--agents-hero-reveal-duration` w CSS nie wydłużała czasu przed CTA.
- Intro **AGENTS**: streamowanie znaków nie wyłącza się przy `prefers-reduced-motion` (wtedy krótsze ~1,5 s na linię zamiast pojawienia całego bloku naraz).

### Added
- Edytor showcase (`tools/showcase-animation-editor`): import pliku timestampów **Whisper / MYSEN** (`.txt` jak `ASSETS/mysen/mysen_timestamps.txt`) — zastępuje listę słów, łamania linii wg segmentów `[a --> b]`, pole `at` względem **audioStart**; parser `parseWhisperStyleTimestampTxtToWordEntries` w `mysen-timestamp-parse.js`.
- Konsola głównej strony: komenda **`/agents`** — przekierowanie do intro AGENTS (`ASSETS/agents/`); wpis w `/help` i `/help full`; w menu Software podpowiedź „Type in terminal: /agents”.
- Podstrona **AGENTS** (`ASSETS/agents/`): intro ze streamowanym tekstem (PL), zanik pierwszego bloku, dwa kolejne akapity i CTA do symulacji (`simulacja-www`); `ASSETS/agents/sim/` przekierowuje do `simulacja-www/`; wpis w menu terminala `>> A G E N T S` otwiera intro.
- **AGENTS · symulacja** (`ASSETS/agents/simulacja-www/`): odtwarzanie sesji; `usePageBaseForHttpApi` w `config.js` + logika w `simulation-replay.js` (baza HTTP = katalog strony przy TOPKEK); katalog `recordings/`.
- Dokumentacja: **vis-network** (9.1.9, jsDelivr) w `WYTYCZNE_PROJEKTU.md`; opis `usePageBaseForHttpApi` w `ASSETS/agents/simulacja-www/IMPLEMENTACJA.md`.

### Fixed
- `start.bat`: serwer uruchamiany w osobnym oknie konsoli z katalogiem projektu (`START /D`), krótkie opóźnienie przed otwarciem przeglądarki — usuwa wyścig „strona przed nasłuchem” na porcie 8002.
- `start.bat` / `server.py`: otwieranie i podpowiedź URL **`http://127.0.0.1:8002/`** zamiast `localhost` — na Windows `localhost` często idzie w IPv6 (`::1`), a `HTTPServer` nasłuchuje IPv4, co kończyło się stroną błędu Chrome (`chrome-error://chromewebdata/`).

### Changed
- Upożądkowano katalog główny: moduły showcase/MYSEN przeniesiono do `src/showcase/`, konsola / FX / Buuch do `src/ui/`; zaktualizowano importy w `script.js` i w `tools/`; `docs/VAJBUJ-instrukcja-zwrotki.md`; rozszerzono `.gitignore` o `/debug-*.log`.

### Added
- Showcase **schema v2**: opcjonalne `postprocessKeyframes`, `volumetricKeyframes`, `customKeyframes` (mapy torów `{ t, v }`), `cameraPreview.fovDeg`; słowo: `wordSize` / `wordHeight` / `wordThickness` / `scatterRadius`, `materialPresetId`, `wordFxId`, `visibleFromFragSec` / `visibleToFragSec`; adapter `voxelLyricsVajbuj` w walidacji (edycja/eksport; MYSEN runtime wymaga `voxelLyricsMysen`); `migrateShowcaseAnimationDocToV2`, `normalizeShowcaseAnimationDocForRuntime`; moduły `showcase-animation-interpolate.js`, `showcase-pack-schema.js`, `showcase-effect-catalog.js`, `showcase-animation-pass-runtime.js`.
- MYSEN runtime: normalizacja dokumentu po `fetch`, klucze postprocessu (bloom/CRT/SAO), volumetryka (`mysenBgCubes` × env map tła), custom w `_showcaseCustomScalars`; okno widoczności słów na osi fragmentu w `stepLyricWordsShared`; `music-lyric-voxels.js`: cache i geometria per słowo + presety materiału `standard` / `neon` / `matte`; kolejka pregen MYSEN z kluczem geometrii.
- Edytor showcase: przebudowa UI (zakładki słowa/postproc/volumetryka/custom), dropdown źródeł, timeline z zoomem i 60 FPS, transport nad osią, siatka XZ i FOV z dokumentu, import/eksport **folderu** (File System Access API) + fallback JSON; moduły `tools/editor-project-io.js`, `editor-timeline.js`, `editor-preview-scene.js`; przykład [ASSETS/vajbuj/showcase-animation.sample.json](ASSETS/vajbuj/showcase-animation.sample.json).
...
### Changed
- `showcase-animation-schema.js`: `SHOWCASE_ANIMATION_SCHEMA_VERSION` = 2 (nowe dokumenty); nadal obsługiwany **v1** przy wczytywaniu; `createEmptyShowcaseAnimationDoc` zwraca v2.
- `tools/README.md`: opis edytora v2, pakietu `showcase-pack.json`, FSA API i ograniczenia adaptera w MYSEN.
- Edytor showcase (`showcase-animation-editor.css`): całość UI mieści się w oknie (`100vh`/`100dvh`, `overflow: hidden` na `body`, `flex: 1 1 0` i `min-height: 0` na siatce / środkowej kolumnie / kanwie podglądu, przewijanie w bocznych panelach); lista słów bez osobnego limitu `vh`; na wąskim ekranie przywrócono przewijanie strony.
